...
public class FTPManager {  
    ...
    Properties properties = CommonUtil.loadConfig(null, C.FILE_PATH);
    public FTPManager() {  
    ...
    public synchronized boolean connect() throws Exception {  
        boolean bool = false;  
        if (ftpClient.isConnected()) {//判断是否有效
            ftpClient.disconnect();
//            return true;
        }  
        ftpClient.setDataTimeout(20000);//设置连接超时时间  
        ftpClient.setControlEncoding("utf-8");  
        ftpClient.connect(properties.getProperty(C.FTP_IP,""), Integer.valueOf(properties.getProperty(C.FTP_PORT,"0")));
        if (FTPReply.isPositiveCompletion(ftpClient.getReplyCode())) {  
            if (ftpClient.login(properties.getProperty(C.FTP_USER_NAME,""), properties.getProperty(C.FTP_PASS_WORD,""))) {
                bool = true;  
